...
class Settings(BaseSettings):
    ...
    celery_broker_url: str = Field("redis://127.0.0.1:6379/1", validation_alias="CELERY_BROKER_URL")
    celery_result_backend: str = Field("redis://127.0.0.1:6379/2", validation_alias="CELERY_RESULT_BACKEND")
    milvus_host: str = Field("127.0.0.1", validation_alias="MILVUS_HOST")
    milvus_port: int = Field(19530, validation_alias="MILVUS_PORT")
    milvus_collection: str = Field("customer_service_chunks", validation_alias="MILVUS_COLLECTION")
    ...
    document_storage_path: str = Field("backend/storage/documents", validation_alias="DOCUMENT_STORAGE_PATH")
