...
export const useChatStore = defineStore("chat", () => {
  ...
  async function listSessions() {
    const response = await fetch("/api/v1/sessions", { headers: authHeaders() });
    if (!response.ok) throw new Error("SESSIONS_LOAD_FAILED");
  ...
  async function updateSession(sessionId: string, title: string) {
    ...
    if (!response.ok) {
      if (response.status === 422) throw new Error("SESSION_TITLE_INVALID");
      if (response.status === 404) throw new Error("SESSION_NOT_FOUND");
      throw new Error("SESSION_UPDATE_FAILED");
    }
    const item = await response.json();
    const index = sessions.value.findIndex((session) => session.id === sessionId);
    if (index >= 0) sessions.value[index] = item;
    return item;
  }

  async function loadMessages(sessionId: string) {
    const response = await fetch(`/api/v1/sessions/${sessionId}/messages`, { headers: authHeaders() });
  ...
  async function deleteSession(sessionId: string) {
    const response = await fetch(`/api/v1/sessions/${sessionId}`, { method: "DELETE", headers: authHeaders() });
    if (!response.ok && response.status !== 204) throw new Error("SESSION_DELETE_FAILED");
  ...
  async function streamQuestion(sessionId: string, question: string, onEvent: (event: SSEEvent) => void) {
