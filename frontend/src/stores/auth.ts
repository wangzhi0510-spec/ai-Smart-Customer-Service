...
export const useAuthStore = defineStore("auth", () => {
  const token = ref<string | null>(globalThis.localStorage?.getItem("access_token") ?? null); const user = ref<any | null>(null); const isAuthenticated = computed(() => Boolean(token.value)); let api: AuthApi = {
    login: async (identifier, password) => (await fetch("/api/v1/auth/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ identifier, password }) })).json(),
    register: async (identifier, password) => (await fetch("/api/v1/auth/register", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ identifier, password }) })).json(),
  };
